...
# Random Forest hyperparameters (tweak if you want)
RF_NUM_TREES = 100
RF_MAX_DEPTH = 8
RANDOM_SEED = 42
TRAIN_RATIO = 0.8
...
# Start Spark
# ----------------------------
spark = SparkSession.builder.appName("FraudDetection_RF_NoLeak").getOrCreate()
...
def main():
    try:
        ...
        # ----------------------------
        # Train/test split using prescriber_id hashing to avoid leakage
        # ----------------------------
        ...
        print("\nTraining Random Forest (no leakage) ...")
        ...
        print("\n===== MODEL PERFORMANCE (Random Forest No Leakage) =====")
        ...
        final_cols = [
            ...
            # intentionally excluding peer_deviation_score and elderly_focus_flag
            "fraud_risk_category", "predicted_category"
        ]

        sorted_preds = predictions.orderBy(col("prescriber_id").asc())
        sorted_preds.select(*[c for c in final_cols if c in predictions.columns]) \
        ...
        if HAS_PLOTTING:
            try:
                ...
                os.makedirs(os.path.dirname(CONFUSION_PNG), exist_ok=True)
